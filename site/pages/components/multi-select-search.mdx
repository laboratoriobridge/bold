...
If the number of pillboxes exceed the area of the component, use a **![Tag](/components/tag.mdx)** to indicate the number of hiding selected items (ex.: [+3]). 

## Best practices

- Order the list by frequency of use, and if applicable, in ascending order.
- If the list show less than 5 items, consider displaying directly on the screen, using the **![Checkbox](/components/checkbox.mdx)** component.
...
- For situation where the user can select only one option, use **![Select](/components/select.mdx)** or **![Radio Button](/components/radio-button.mdx)**.
