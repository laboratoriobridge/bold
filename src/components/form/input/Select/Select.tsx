...
export interface SelectProps<T = DefaultItemType> extends DownshiftProps<T>, WithStylesProps {
    /**
...
export class Select<T> extends React.PureComponent<SelectProps<T>, SelectState<T>> {
    ...
    render() {
        const { css, theme, items, filter, renderItem, disabled, onBlur, status, ...rest } = this.props
